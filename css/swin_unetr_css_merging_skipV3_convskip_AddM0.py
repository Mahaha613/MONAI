...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        if self.use_1x1_conv_for_skip:
            ...
            self.conv_1x1x1_m0 = Convolution(
                spatial_dims=3,
                in_channels=2*feature_size,
                out_channels=feature_size,
                kernel_size=1,
...
# *******************************************************************
    ...
    def css_add_conv_skip(self, x_in, conv_layer):
        output = conv_layer(x_in)
        return output
    ...
    def forward(self, x_in):  
        ...
        if self.css_skip:
            m4 = self.css_add_conv_skip(enc3, self.conv_skip_m4)
            if self.use_1x1_conv_for_skip:
            ...
            m3 = self.css_add_conv_skip(enc2, self.conv_skip_m3)  # m3:torch.Size([4, 96, 12, 12, 4])
            if self.use_1x1_conv_for_skip:
                enc3 = self.conv_1x1x1_m3(torch.cat([enc3, m3], dim=1))
            else:
                enc3 = torch.cat([enc3, m3], dim=1)

            m2 = self.css_add_conv_skip(enc1, self.conv_skip_m2)
            if self.use_1x1_conv_for_skip:
                enc2 = self.conv_1x1x1_m2(torch.cat([enc2, m2], dim=1))
            else:
                enc2 = torch.cat([enc2, m2], dim=1)

            m1 = self.css_add_conv_skip(enc0, self.conv_skip_m1)  # no need to double the channels
            if self.use_1x1_conv_for_skip:
                enc1 = self.conv_1x1x1_m1(torch.cat([enc1, m1], dim=1))
            else:
                enc1 = torch.cat([enc1, m1], dim=1)

            m0 = self.css_add_conv_skip(x_in, self.conv_skip_m0)  # no need to double the channels
            if self.use_1x1_conv_for_skip:
                enc0 = self.conv_1x1x1_m0(torch.cat([enc0, m0], dim=1))
            else:
                enc0 = torch.cat([enc0, m0], dim=1)
